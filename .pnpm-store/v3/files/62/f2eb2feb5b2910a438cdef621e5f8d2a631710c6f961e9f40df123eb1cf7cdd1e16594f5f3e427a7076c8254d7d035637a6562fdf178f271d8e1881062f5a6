...
var STDOUT = "<stdout>";
function shouldInjectOutput(path$1) {
	return path$1 === STDOUT || require_core.isJsFile(path$1);
}
/**
...
*/
function canonicalPath(base, path$2) {
	const resolved = (0, path.resolve)(base, path$2);
	try {
		return (0, fs.realpathSync)(resolved);
	} catch (_) {
		return resolved;
	}
}
...
function entryOutputPaths(metafile, initialOptions) {
	...
	const paths = /* @__PURE__ */ new Set();
	for (const [outputPath, output] of Object.entries(metafile.outputs)) if (output.entryPoint && entryPoints.has(canonicalPath(base, output.entryPoint))) paths.add((0, path.resolve)(base, outputPath));
	return paths;
}
			...
			build.onEnd((result) => {
				...
				if (result.outputFiles) {
					for (const file of result.outputFiles) {
						...
						if (file.path !== STDOUT && !entryPaths.has((0, path.resolve)(file.path))) continue;
