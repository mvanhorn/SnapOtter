const require_core = require("./core-DCHzVZ6n.cjs");
...
function codeTransformerRollup(options) {
	const { transform: transformCode, getCodeToInject } = require_core.createCodeTransformer(options);
	let sourcemapsEnabled = false;
	const outputOptions = (inputOptions) => {
		sourcemapsEnabled = !!inputOptions.sourcemap;
	};
	const renderChunk = (code, chunk, _, meta) => {
		if (!require_core.isJsFile(chunk.fileName)) return null;
		if (!chunk.isEntry) return null;
		if (require_core.shouldSkipCodeInjection(code, chunk.facadeModuleId)) return null;
		...
		const match = code.match(require_core.COMMENT_USE_STRICT_REGEX)?.[0];
		if (match) ms.appendLeft(match.length, injectCode);
		else ms.prepend(injectCode);
	...
	function transformHandler(code, id) {
		const result = transformCode(code, id, sourcemapsEnabled ? this.getCombinedSourcemap() : void 0);
	...
	const idFilter = options.transformFilter === void 0 ? /node_modules/ : options.transformFilter;
	const transform = idFilter === false ? transformHandler : {
		filter: { id: idFilter },
...
//#endregion
Object.defineProperty(exports, "codeTransformerRollup", {
	enumerable: true,
	get: function() {
		return codeTransformerRollup;
	}
});

//# sourceMappingURL=rollup-BovaE4iR.cjs.map