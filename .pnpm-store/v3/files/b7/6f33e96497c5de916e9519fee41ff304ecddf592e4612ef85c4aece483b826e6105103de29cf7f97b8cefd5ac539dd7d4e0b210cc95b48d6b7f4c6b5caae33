...
function getPagefindHead(base) {
  return [
    [
      ...
      `
        const load = () => {
          import('${(0, import_theme_shared.withBase)(base || "", "/pagefind/pagefind.js")}')
            .then((module) => {
...
var aliasSearchVueFileMPA = `${getDirname()}/../src/SearchMPA.vue`;
var aliasSearchVueFileMPADefault = `${getDirname()}/../src/SearchMPADefault.vue`;
...
function pagefindPlugin(searchConfig = {}) {
  ...
  const pluginOps = {
    ...
    async configResolved(config) {
      ...
      if (!searchConfig.manual) {
        await (0, import_theme_shared2.cacheAllGitTimestamps)(vitepressConfig.srcDir);
      }
    ...
    async transform(code, id, options) {
      ...
      const filepath = isDynamicRoute ? (0, import_theme_shared2.joinPath)(vitepressConfig.srcDir, `/${dynamicRoute.route}`) : pathname;
      const isExist = import_node_fs.default.existsSync(filepath);
      if (!isExist) {
      ...
      if (!searchParams.size || searchParams.has("lang.ts")) {
        ...
        const { data: frontmatter, content } = (0, import_theme_shared2.grayMatter)(fileContent, {
          excerpt: true
        });
        if (!content.trim()) {
          return code;
        ...
        if (!searchConfig.manual) {
          ...
          frontmatter.date = +new Date(frontmatter.date || await (0, import_theme_shared2.getFileLastModifyTime)(cleanId, vitepressConfig.cacheDir));
